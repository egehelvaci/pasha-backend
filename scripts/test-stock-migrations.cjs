...
(async()=>{
 ...
 try {
  for(const name of ['20260924040000_product_aliases','20260924050000_cart_stock_reservations','20260924060000_cart_reservation_lock_order','20260924070000_reservation_alias_lock']) {
  ...
  const product=(await client.query('SELECT product_id FROM product_stocks LIMIT 1')).rows[0];
  ...
  const item=(await client.query('INSERT INTO cart_items(cart_id,product_id,quantity,width,height,area_m2,unit_price,total_price) VALUES($1,$2,2,100,200,2,1,4) RETURNING id',[cart.id,product.product_id])).rows[0];
  ...
  assert.equal(await reserved(),before+4);
  await client.query('UPDATE cart_items SET quantity=3 WHERE id=$1',[item.id]);assert.equal(await reserved(),before+6);
  await client.query('UPDATE carts SET is_active=false WHERE id=$1',[cart.id]);assert.equal(await reserved(),before);
  await client.query('UPDATE carts SET is_active=true WHERE id=$1',[cart.id]);assert.equal(await reserved(),before+6);
  await client.query('DELETE FROM cart_items WHERE id=$1',[item.id]);assert.equal(await reserved(),before);

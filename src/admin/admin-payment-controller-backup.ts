      ...
      return res.status(200).json({
        ...
        data: {
          ...
          summary: {
            ...
            totalAmount: Number(totalAmount._sum.amount || 0),
            successRate: totalCount > 0 ? Math.round((completedCount / totalCount) * 100) : 0,
            // Para birimi bazında detaylar
            tryPayments: {
              count: tryPaymentsCount,
              totalAmount: Number(tryPaymentsTotal._sum.original_amount || 0)
            },
            usdPayments: {
              count: usdPaymentsCount,
              totalAmount: Number(usdPaymentsTotal._sum.original_amount || 0)
            }
          },
          store: {

...
async function fixQRCodes() {
  ...
  try {
    ...
    // Her item için 1 QR kod oluştur, required_scans = quantity
    ...
    for (const item of order.items) {
      ...
      console.log(`   Miktar: ${item.quantity} (${item.quantity} kere okutulmalı)`)
      console.log(`   QR: ${createdQRCode.qr_code}`)
      console.log(`   ---`)
    }
    ...
    createdQRCodes.forEach((qr, index) => {
      ...
      console.log(`   ${index + 1}. ${item.product.name} → ${item.quantity} kere okutulmalı`)
    })
